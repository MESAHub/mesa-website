...
<h3 id="use_eosPTEH_with_eosDT">use_eosPTEH_with_eosDT <a href="#use_eosPTEH_with_eosDT" title="Permalink to this location">¶</a></h3>


overwrite the control of same name in the `Eos_General_Info` structure

{% highlight fortran %}
use_eosPTEH_with_eosDT = .false.
{% endhighlight %}
...
Note that these parameters are not exactly the same as used by Paul Duffell.
His calibrated D is 2, where MESA uses D = 3 (see MESA IV).
